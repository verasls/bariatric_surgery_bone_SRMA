...
#
# Compute the standard deviation from either the standard error, the
# confidence interval or the interquartile range.
#
# Args:
#   from: A character string indicating from which statistic to compute the
# standard deviation from. Can be either "se", "ci" or "iqr".
#   n: The sample size.
...
compute_sd <- function(from, n, mean, x_lower = NA, x_upper = NA) {
  if (from == "se") {
    sd_from_se(n, mean, x_lower, x_upper)
  } else if (from == "ci") {
...
sd_from_se <- function(n, mean, se_lower, se_upper) {
  sem1 <- mean - se_lower
  sem2 <- se_upper - mean
  sem <- (sem1 + sem2) / 2
  sem * sqrt(n)
