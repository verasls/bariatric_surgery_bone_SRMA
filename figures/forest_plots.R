...
make_LS_plot <- function() {
  ...
  # X axis label
  text(15, -5.5, font = 1, pos = 2, cex = 1, "Mean percentage change")
  # Plot title
  text(
...
p1 <- as.ggplot(~ make_radius_plot())
p2 <- as.ggplot(~ make_tibia_plot())
p3 <- as.ggplot(~ make_LS_plot())
p4 <- as.ggplot(~ make_TH_plot())
agg_png(
...
plot_grid(p1, p2, p3, p4, ncol = 1, rel_heights = c(1, 1, 0.95, 0.6))
dev.off()
